...
def moved_object_scene_name(label: str) -> str:
    """Return a namespaced scene identifier for the benchmark moved object."""
    clean_label = str(label).strip()
    if not clean_label:
        raise ValueError("moved object label must be non-empty")
    return f"{MOVED_OBJECT_SCENE_PREFIX}{clean_label}"


def is_moved_object_scene_name(name: str) -> bool:
    """Return True when a scene name belongs to the benchmark moved object."""
