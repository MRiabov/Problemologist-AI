...
class GenesisBackend(PhysicsBackend):
    ...
    def _to_flat_list(val):
        """Standardized conversion from Genesis/PyTorch tensors to CPU lists."""
        if hasattr(val, "cpu"):
            val = val.cpu().numpy()
    ...
    def get_actuator_state(self, actuator_name: str) -> ActuatorState:
        ...
        if entity_name in self.entities:
            ...
            try:
                ...
                if motor_config and "physics" in motor_config:
                    torque = motor_config["physics"]["torque"]
                    forcerange = (-torque, torque)
