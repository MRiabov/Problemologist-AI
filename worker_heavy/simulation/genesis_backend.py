...
class GenesisBackend(PhysicsBackend):
    ...
    def _to_flat_list(val):
        """Helper to convert Genesis tensors/arrays to flat lists.
                ...
                if entity:
                    try:
                        ...
                        if joint:
                            ...
                            # Clamp value to force_range if present
                            force_range = info.get("force_range")
                            if force_range:
                                val = float(
                                    np.clip(val, force_range[0], force_range[1])
                                )
