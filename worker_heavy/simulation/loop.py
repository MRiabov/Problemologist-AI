...
class SimulationLoop:
    ...
    def _check_simulation_failure(
        ...
        fail_mode = self.objective_evaluator.update(
            self.backend, current_time, dt_interval, getattr(self, "current_step_idx", 0)
        )
        if fail_mode:
            self.fail_reason = fail_mode
            return True
        ...
        for bname in self.body_names:
            ...
            eval_fail = self.success_evaluator.check_failure(
                current_time, bstate.pos, bstate.vel
            )
            if eval_fail:
                if eval_fail == SimulationFailureMode.OUT_OF_BOUNDS:
                    logger.warning(
        ...
        # 7. Motor overload
        if self._check_motor_overload(dt_interval):
            self.fail_reason = SimulationFailureMode.MOTOR_OVERLOAD
            return True

        return False

    def _resolve_backend_failure(self, res) -> SimulationFailureMode:
        """Translate backend failure into SimulationFailureMode."""
        if isinstance(res.failure_reason, SimulationFailureMode):
            return res.failure_reason
        ...
        for wire in self.electronics.wiring:
            if getattr(wire, "routed_in_3d", False):
                try:
                    ...
                    if tension > limit:
                        self.fail_reason = (
                            f"{SimulationFailureMode.WIRE_TORN.value}:{wire.wire_id}"
                        )
                        emit_event(
                            ElectricalFailureEvent(
                                ...
                                message=(
                                    f"Wire {wire.wire_id} torn due to high tension"
                                ),
                            )
                        )
                        wire_broken = True
