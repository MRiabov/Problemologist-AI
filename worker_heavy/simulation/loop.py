    ...
    def step(
        ...
        for step_idx in range(steps):
            ...
            if step_idx % check_interval == 0 or step_idx == steps - 1:
                ...
                if self.objectives and self.objectives.objectives:
                    # Only check if we have flow rate objectives
                    has_flow_rate = any(
                            ...
                            for i, fo in enumerate(
                                ...
                                if fo.type == FluidObjectiveType.FLOW_RATE:
                                    ...
                                    if (
                                        ...
                                        and len(prev_distances)
                                        == len(current_distances)
                                    ):
                                        # Detect crossing: sign change means particle crossed plane
                                        # We count crossings in the direction of the normal (negative -> positive)
                                        # assuming normal aligns with flow.
                                        # Behind: dot < 0. Front: dot > 0.
                                        # Crossing: prev < 0 and curr >= 0.

                                        # Count positive crossings (aligned with normal)
                                        ...
                                        self.cumulative_crossed_count[
                                            obj_id
                                        ] = self.cumulative_crossed_count.get(
                                            obj_id, 0
                                        ) + count
