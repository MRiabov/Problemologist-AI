...
def upgrade() -> None:
    ...
    op.execute("CREATE TYPE assettype AS ENUM ('VIDEO', 'MJCF', 'IMAGE', 'STEP', 'STL', 'GLB', 'PYTHON', 'OTHER')")
    ...
    op.execute("CREATE TYPE episodestatus AS ENUM ('RUNNING', 'PLANNED', 'COMPLETED', 'FAILED', 'CANCELLED')")
    op.execute("UPDATE episodes SET status = UPPER(status)")
    op.execute("ALTER TABLE episodes ALTER COLUMN status TYPE episodestatus USING status::episodestatus")
    ...
    op.execute("CREATE TYPE tracetype AS ENUM ('TOOL_START', 'TOOL_END', 'LLM_START', 'LLM_END', 'LOG', 'ERROR', 'EVENT')")
    op.execute("UPDATE traces SET trace_type = UPPER(trace_type)")
    op.execute("ALTER TABLE traces ALTER COLUMN trace_type TYPE tracetype USING trace_type::tracetype")
