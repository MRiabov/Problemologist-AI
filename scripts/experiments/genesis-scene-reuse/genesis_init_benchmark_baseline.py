...
def _run_spawned_rounds(
    rounds: int, backend_name: str, include_scene: bool
) -> dict[str, Any]:
...
def _thread_scene_worker(
    result: dict[str, Any], backend_name: str, do_init: bool
) -> None:
    try:
        _configure_headless_env()
        import genesis as gs
...
def _thread_reuse_probe_child(backend_name: str) -> dict[str, Any]:
    _configure_headless_env()
    import genesis as gs
    ...
    t = threading.Thread(
        ...
        args=(after_main_init, backend_name, False),
    ...
    proc_result = _run_spawned_thread_init_probe(backend_name)
...
def _thread_init_probe_child(backend_name: str) -> dict[str, Any]:
    result: dict[str, Any] = {}
    worker = threading.Thread(
        target=_thread_scene_worker,
        args=(result, backend_name, True),
...
def _run_spawned_thread_init_probe(backend_name: str) -> dict[str, Any]:
...
def _run_same_process_reuse(rounds: int, backend_name: str) -> dict[str, Any]:
    return _run_child_subprocess(
...
def _run_thread_reuse_probe(backend_name: str) -> dict[str, Any]:
    return _run_child_subprocess(
...
def main() -> None:
    ...
    report = {
        ...
        "thread_reuse_probe": _run_thread_reuse_probe(args.backend),
    }
