...
def measure_compilation():
    ...
    # 4. Mesh: Box Mesh (build123d export)
    print("\nPhase 3: Box Mesh (build123d export)")
    # Create a temp mesh
    part = Box(10, 10, 10)
    stl_path = "temp_box.stl"
    obj_path = "temp_box.obj"
    ...
    print(f"  Scene Build (Box Mesh Voxelization): {build_duration:.2f}s")
    ...
    print("\nPhase 4: Sphere Mesh (build123d export, default tolerance)")
    part = Sphere(10)
    ...
    # 6. Mesh: Sphere (Optimized for smoke test)
    print("\nPhase 5: Sphere Mesh (tolerance=1.0, smoke test optimization)")
    part = Sphere(10)
    stl_path = "temp_sphere_coarse.stl"
    obj_path = "temp_sphere_coarse.obj"
    export_stl(part, stl_path, tolerance=1.0, angular_tolerance=1.0)
    mesh = trimesh.load(stl_path)
    mesh.export(obj_path)
    ...
    print(f"  Scene Build (Coarse Sphere Mesh Voxelization): {build_duration:.2f}s")
