...
from controller.generators.benchmark.nodes import coder_node, extract_python_code
from controller.generators.benchmark.models import GenerationSession
from controller.generators.benchmark.state import BenchmarkGeneratorState
...
async def test_coder_node_success(mock_state):
    ...
    with patch("controller.generators.benchmark.nodes.ChatOpenAI") as mock_llm_class:
...
async def test_coder_node_with_feedback(mock_state):
    ...
    mock_state["simulation_result"] = {"valid": False, "cost": 0, "logs": ["Intersections found"]}
    ...
    with patch("controller.generators.benchmark.nodes.ChatOpenAI") as mock_llm_class:
        ...
        with patch("controller.generators.benchmark.nodes.Path.read_text") as mock_read:
...
async def test_validator_node_success(mock_state):
    from controller.generators.benchmark.nodes import validator_node
