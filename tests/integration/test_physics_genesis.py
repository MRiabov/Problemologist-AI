...
from shared.simulation.backends import SimulatorBackendType, StepResult, StressField
...
def mock_genesis_backend():
    with patch("worker.simulation.factory.get_physics_backend") as mock_get:
...
def test_fluid_containment_integration(mock_genesis_backend, tmp_path):
    ...
    objectives = ObjectivesYaml(
        ...
        fluids=[
            FluidDefinition(
                fluid_id="water",
                initial_volume=FluidVolume(type="box", center=(0.5, 0.5, 0.5), size=(0.1, 0.1, 0.1))
            )
        ]
    )
...
def test_fluid_containment_failure(mock_genesis_backend, tmp_path):
    ...
    mock_genesis_backend.get_particle_positions.return_value = np.vstack([inside, outside])
    mock_genesis_backend.get_stress_field.return_value = StressField(
        nodes=np.zeros((1, 3)),
        stress=np.array([100e6])
    )
    ...
    assert "FLUID_OBJECTIVE_FAILED:water" in metrics.fail_reason
...
def test_part_breakage_integration(mock_genesis_backend, tmp_path):
    ...
    assert metrics.success is False
    assert metrics.fail_reason is not None
    assert "PART_BREAKAGE:bucket" in metrics.fail_reason
