...
def test_gpu_oom_retry_logic(tmp_path):
    ...
    with (
        ...
        # Verify that the retry loop was created with smoke_test_mode=True
        # mock_loop_cls is the SimulationLoop class mock
        _, last_call_kwargs = mock_loop_cls.call_args
        assert last_call_kwargs.get("smoke_test_mode") is True
