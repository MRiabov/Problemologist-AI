...
def test_cnc_reuse_discount():
    ...
    # 1. First part cost
    context = {}
    cost1 = workbench.calculate_cost(part, quantity=1, context=context)
    
    # 2. Identical part cost (same center/volume)
    cost2 = workbench.calculate_cost(part, quantity=1, context=context)
...
def test_im_reuse_discount():
    ...
    # 1. First part cost
    context = {}
    cost1 = workbench.calculate_cost(part, quantity=100, context=context)
    
    # 2. Identical part cost
    cost2 = workbench.calculate_cost(part, quantity=100, context=context)
