...
def test_search_cots_catalog_tool(mock_parts):
    with patch("shared.cots.agent.search_parts") as mock_search:
        ...
        # Tools in LangChain are invoked via .invoke(), but this is a plain function
...
def test_create_cots_search_agent():
    # Mocking dspy.CodeAct is simpler than mocking ChatOpenAI in this context
    # since create_cots_search_agent uses dspy.CodeAct internally
    with patch("shared.cots.agent.dspy.CodeAct") as mock_codeact:
        agent = create_cots_search_agent("gpt-4o")
        assert agent is not None
        mock_codeact.assert_called_once()
