...
def test_run_linter_syntax_error():
    ...
    assert any(e["linter"] == "ruff" for e in errors) or any(
        e["linter"] == "pyrefly" for e in errors
    )
...
def test_run_linter_filtering_ruff():
    ...
    assert len(errors) == 1
    assert "undefined_func" in errors[0]["message"]
    assert "unused" not in errors[0]["message"].lower()
...
def test_run_linter_pyrefly_filtering(mock_run):
    ...
    assert "warning" not in errors[0]["message"].lower()


def test_format_linter_report():
    issues = [
        {"linter": "ruff", "line": 4, "code": "F401", "message": "import os is unused"},
        {
            "linter": "pyrefly",
            "line": 12,
            "code": "type-error",
    ...
    report = format_linter_report(issues)
    assert "Linter Feedback:" in report
    assert "Line 4: ruff says: import os is unused (F401)" in report
    assert "Line 12: pyrefly says: Type mismatch (type-error)" in report
