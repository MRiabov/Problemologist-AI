...
def test_transcript_multi_node_sequence(mock_scenarios):
    ...
    with patch.object(MockDSPyLM, "_load_scenarios", return_value=mock_scenarios):
        ...
        lm = MockDSPyLM(session_id="TRANSCRIPT-TEST", node_type="planner")
        ...
        assert "[[ ## next_tool_args ## ]]\n{\"path\": \"plan.md\", \"content\": \"Initial plan\"}" in resp[0]
        ...
        # 3. Coder Turn
        lm_coder = MockDSPyLM(session_id="TRANSCRIPT-TEST", node_type="coder")
...
def test_transcript_fallback_to_legacy(mock_scenarios):
    ...
    with patch.object(MockDSPyLM, "_load_scenarios", return_value=mock_scenarios):
        lm = MockDSPyLM(session_id="LEGACY-TEST", node_type="planner")
