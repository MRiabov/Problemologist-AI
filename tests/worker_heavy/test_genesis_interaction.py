...
def test_apply_control(genesis_backend):
    ...
    # Check if control_dofs_force (newer Genesis) or set_dofs_force (older) was called
    call_args = (
        mock_entity.control_dofs_force.call_args or mock_entity.set_dofs_force.call_args
    )
    assert (
        call_args is not None
    ), "Neither control_dofs_force nor set_dofs_force was called"
