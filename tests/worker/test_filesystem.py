...
from worker.filesystem.backend import SandboxFilesystemBackend, SimpleSessionManager, FileInfo
from worker.filesystem.router import FilesystemRouter, MountPoint, AccessMode, WritePermissionError
...
def mock_fs():
    ...
    fs.ls.side_effect = lambda p, detail=False: [{"name": k, "type": "file", "size": len(v)} for k, v in storage.items() if k.startswith(p.rstrip("/") + "/")]
...
def test_filesystem_router_logic(mock_fs, tmp_path):
    ...
    mounts = [
        MountPoint(virtual_prefix="/utils", local_path=local_utils, access_mode=AccessMode.READ_ONLY)
    ]
