...
async def test_asset_content_persistence(db_session):
    # Create an episode
    episode = Episode(id=uuid.uuid4(), task="test task", status=EpisodeStatus.COMPLETED)
...
async def test_episode_relationships(db_session):
    ...
    asset1 = Asset(
        episode_id=episode_id, asset_type=AssetType.IMAGE, s3_path="img1.png"
    )
    asset2 = Asset(
        episode_id=episode_id, asset_type=AssetType.VIDEO, s3_path="vid1.mp4"
    )
