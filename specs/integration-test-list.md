...
| INT-190 | Seeded render evidence sanity gate | Seeded entry validation rejects blank/black render images for seeded workspaces, including engineer-coder `ec-001`, and accepts visible render images; the CLI seed validator and shared preflight helper must agree on the fail-closed result. |
...
| INT-190 | P1 | Seeded render evidence sanity gate | Feed a seeded engineer-coder workspace with blank and visible render assets through `scripts/validate_eval_seed.py` and the shared seeded-entry preflight; assert blank images fail closed, visible images pass, and both paths report the same render-validation outcome. |
...
| INT-188 | Call `/benchmark/validate` through the live heavy-worker path with `physics.backend=GENESIS`; assert build123d/VTK-backed preview artifacts, `renders/render_manifest.json`, objective-box overlays, modality-toggle behavior, fail-closed rejection when all preview modalities are disabled, depth/segmentation overlay visibility, and no extra Genesis parity gate in the validation path. | Static artifact-file snapshot checks without executing the live validation path or asserting only backend-selection helpers. |
...
- `tests/integration/architecture_p1/`: INT-034, INT-160..INT-179, INT-190, INT-207, INT-208, INT-210, INT-211.
