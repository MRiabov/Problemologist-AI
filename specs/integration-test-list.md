...
| INT-204 | Run benchmark generation over HTTP and assert benchmark plan reviewer inspects the current revision benchmark preview evidence via `inspect_media(...)` before approval, with persisted stage-specific decision/comments YAML tied to the latest revision. | Asserting only file presence or text summaries without media-inspection traces. |
