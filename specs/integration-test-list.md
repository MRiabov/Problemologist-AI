...
| INT-188 | Call `/benchmark/validate` through the live heavy-worker path with `physics.backend=GENESIS`; assert build123d/VTK-backed preview artifacts, `renders/render_manifest.json`, objective-box overlays, modality-toggle behavior, fail-closed rejection when all preview modalities are disabled, depth/segmentation overlay visibility, and no extra Genesis parity gate in the validation path. The preview path must source benchmark assembly via `build()` and objective overlays via `objectives_geometry()` from `benchmark_script.py` before composing benchmark context. | Static artifact-file snapshot checks without executing the live validation path or asserting only backend-selection helpers. |
