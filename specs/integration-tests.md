...
| INT-071 | Execute per-agent file operations via live file APIs and assert `agents_config.yaml` precedence (`deny` > `allow`, unmatched deny, agent override), strict deny of `.manifests/**` to all agent roles, and reviewer-only stage-specific write scopes (`reviews/benchmark-plan-review-round-*.md`, `reviews/benchmark-review-round-*.md`, `reviews/engineering-plan-review-round-*.md`, `reviews/engineering-execution-review-round-*.md`, `reviews/electronics-review-round-*.md`). | In-process path policy matcher and precedence helpers only. |
