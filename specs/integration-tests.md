...
| INT-188 | Call `/benchmark/validate` through the live heavy-worker path with `physics.backend=GENESIS`; assert build123d/VTK-backed preview artifacts, `renders/render_manifest.json`, objective-box overlays, modality-toggle behavior, depth/segmentation overlay visibility, and no extra Genesis parity gate in the validation path. | Static artifact-file snapshot checks without executing the live validation path or asserting only backend-selection helpers. |
