...
| INT-008 | `benchmark_definition.yaml` logic validation | Build/goal/forbid constraints validated: bounds checks, no illegal intersections, valid benchmark-owned fixture metadata (`benchmark_parts` unique IDs/labels, required `material_id` or `cots_id` when metadata is declared, attachment policy uses typed `attachment_methods` with only `fastener`/`none`), mandatory `moved_object.material_id` resolves to a known `manufacturing_config.yaml` material, and benchmark planner estimate fields are schema-valid. |
...
| INT-011 | Planner caps under benchmark caps | Planner-owned `planner_target_max_unit_cost_usd`/`planner_target_max_weight_g` are \<= the benchmark/customer caps derived into `benchmark_assembly_definition.yaml`. |
...
| INT-206 | Failed episode replay bundle contract | `/api/episodes/{episode_id}/replay` reconstructs a failed episode from persisted artifacts and metadata only; repeated replay calls are stable; replay output exposes the original episode/user/worker IDs, terminal metadata, entry-validation context, validation result, simulation result, review manifests, review decisions, trace IDs, and replay artifact hashes; missing replay artifacts, stale manifest revisions, or unsupported mechanisms fail closed with distinct `422` reasons. |
...
| INT-114 | Benchmark planner explicit submission gate | Benchmark planner must emit explicit `submit_plan` (`TOOL_START`) with `node_type=benchmark_planner`; successful submission must materialize `.manifests/benchmark_plan_review_manifest.json`, keep planner-authored benchmark estimate fields normalized in `benchmark_definition.yaml`, require a schema-valid `benchmark_assembly_definition.yaml` whose benchmark/customer caps are derived from those estimates, unblock `Benchmark Plan Reviewer`, and only after benchmark plan-review approval may the episode reach `PLANNED` (and must not transition to `FAILED`). Missing submission fails closed. |
