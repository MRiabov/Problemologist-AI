...
12. Reviewer entry must be gated by a valid latest-revision reviewer-specific handover artifact (for example `.manifests/benchmark_review_manifest.json`, `.manifests/engineering_execution_review_manifest.json`, `.manifests/electronics_review_manifest.json`, or `.manifests/engineering_plan_review_manifest.json` for plan-review stage); tool-trace presence alone must never unlock review.
...
| INT-074 | Engineering DOF minimization review gate | Engineering plan reviewer rejects excessive/unjustified `final_assembly.parts[*].dofs` assignments, and engineering execution reviewer flags over-actuated plan deviations even when a single simulation run passes. |
...
| INT-034 | Reviewer evidence completeness | Review decisions include expected evidence fields and valid reviewer-specific handover manifest (for example `.manifests/benchmark_review_manifest.json`, `.manifests/engineering_execution_review_manifest.json`, or `.manifests/electronics_review_manifest.json`) tied to the latest revision and successful simulation result. |
...
| INT-034 | Submit real reviews and assert evidence completeness plus persisted reviewer-specific manifest linkage (`.manifests/benchmark_review_manifest.json`, `.manifests/engineering_execution_review_manifest.json`, or `.manifests/electronics_review_manifest.json`) for the accepted/rejected latest revision, with refusal when manifest is stale/missing/invalid. | Unit-test of review schema only. |
...
| INT-074 | Run planner->plan-reviewer->coder->execution-reviewer flow over HTTP with seeded over-actuated plans; assert plan reviewer rejects unjustified DOFs and execution reviewer flags over-actuated plan deviations with persisted review evidence/event. | Unit tests that only compare parsed DOF lists or helper outputs without real orchestration/handover boundaries. |
