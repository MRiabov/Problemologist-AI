...
4. For planner handoff artifacts, preflight must additionally run cross-contract semantic checks (engineering attachment/cost checks and benchmark planner semantic checks) equivalent to the runtime handoff gates.
5. If any schema-backed handoff artifact or cross-contract check is invalid, preflight must fail closed and the eval run must not proceed into node execution.
6. Validation ownership remains in controller node-entry validation; evals are orchestration callers and must not duplicate artifact-schema or handoff-semantic validators locally.
