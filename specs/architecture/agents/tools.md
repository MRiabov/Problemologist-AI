...
- `preview_drawing(...)` - async on-demand preview submission for planner drafting technical drawings. The runtime renders the drafting package, 2D projection set, and any vector sidecars needed for review, then persists the result under the same role-scoped render buckets as other preview evidence. The tool is for inspection, not source authoring; `inspect_media(...)` remains the tool for viewing the persisted render files. The implementation may use build123d `TechnicalDrawing`, `project_to_viewport()`, `ExportSVG`, and `ExportDXF`, but those names are implementation details rather than the agent-facing contract. When a workspace starts from the convenience scaffold, that scaffold is the reusable default 3-view orthographic template, not a separate authoring path.
