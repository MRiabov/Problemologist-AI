...
1. circuit validity,
1. wire routing in 3D space,
1. electrical failure modes that can stop a mechanism,
1. reviewer-visible artifacts for both the electrical plan and the physical route.
...
1. Invalid circuits fail before expensive physics simulation runs.
1. Wires can matter physically when the benchmark requires it.
1. Electrical failures propagate into the mechanical result.
1. Reviewers can inspect both the schematic and the routed design.
1. Electrical components and wire costs count toward total solution cost and weight.
...
1. PCB design,
1. firmware or embedded programming,
1. analog signal-integrity analysis,
1. wireless or RF systems,
1. general-purpose electronics CAD as a separate product,
1. broad sensor logic beyond what later work may introduce.
...
1. the engineer provides an electrical design,
1. simulation validates that design before or during motion,
1. reviewers inspect the electrical evidence alongside the mechanical result.
...
1. strict COTS identification still applies to benchmark-owned catalog parts,
1. benchmark-owned electronics do not contribute to engineer manufacturability validation, engineer price, or engineer weight totals,
1. benchmark-owned powered fixtures may be implicitly powered in MVP,
1. benchmark-owned powered fixtures do not require wiring realism unless the benchmark explicitly makes wiring part of the task,
1. benchmark-owned motion still requires reviewer-visible dynamic evidence and a stable handoff contract,
1. benchmark-owned fixtures are validation setup, not engineer-owned solution parts, so they are not required to be manufacturable as final deliverables.
...
1. `Electronics Planner` augments that plan with electrical requirements, component choices, and wiring intent,
1. `Engineering Plan Reviewer` approves the combined handoff,
1. `Engineering Coder` implements the whole approved solution in one revision,
1. `Electronics Reviewer` performs specialist electrical review on the unified implementation,
1. `Engineering Execution Reviewer` performs final post-success execution review.
...
1. connector access can require bracket or frame changes,
1. PSU placement can require packaging and center-of-mass changes,
1. late serialized electrical-only coding creates avoidable planner drift and rework loops.
...
1. impossible power budgets should not waste expensive simulation time,
1. the engineer should receive a crisp electrical failure instead of a vague mechanical miss.
...
1. the electrical layer decides whether it can move,
1. the effective motion is the controller command scaled by electrical power availability.
...
1. the planner handoff declares the concrete electrical solution intent and constraints,
1. the reviewer can judge whether the solution actually satisfies the benchmark instead of whether it simply contains electrical-looking data.

Implementation ownership rule:

1. `assembly_definition.yaml.electronics` is planner-owned after plan approval,
1. `Engineering Coder` implements that approved electrical design in `script.py` and helper modules,
1. implementation changes that require planner-level electrical redesign must route through plan refusal / replanning, not through a separate late electrical implementer node.
...
1. open circuits,
1. floating nodes,
1. current draw above the available supply,
1. wires that are undersized for the requested current when they are used as purely logical conductors.
...
1. under-provisioned designs must fail or be rejected before approval,
1. reviewers must be able to see the margin between required and available current,
1. the planner should choose credible supply headroom rather than infinite-budget electrical designs.
...
1. the mandatory pre-simulation gate remains the simpler validity and power-budget check,
1. we do not require oscilloscope-grade electrical analysis to validate a powered mechanical benchmark.
...
1. the electrical design delivers power.
...
1. this exception is allowed only for read-only benchmark fixtures,
1. engineer-authored powered mechanisms still require explicit electrical design and power validation.
...
1. scaling controller commands by that power map,
1. updating that map when the electrical state changes,
1. propagating electrical failures into the mechanical outcome.
...
1. physical routed wires that can interact with geometry and motion.
...
1. explicit gauge selection,
1. explicit length,
1. a flag that says the route participates in physical simulation.
...
1. moving mechanisms need extra routing care around sweep volumes,
1. the system should enforce a baseline static clearance rule before simulation starts.
...
1. forced through a joint or sweep envelope,
1. built from unstable short zig-zag segments that are clearly not intended as a real harness path.
...
1. gauge-dependent tensile limits,
1. a failure state when a wire tears,
1. loss of power to the affected path after the tear.
...
1. the planner must consider sealing, shielding, or separation as part of the solution,
1. the reviewer must treat fluid exposure as a first-class electrical failure, not a minor warning.
...
1. the electrical side defines power, components, and wiring,
1. the reviewer checks both the electrical validity and the mechanical implications,
1. conflicts can route back from electronics to mechanics when the wire path is infeasible.
...
1. assembly geometry relevant to routing,
1. benchmark electrical constraints,
1. the planner's intended powered behavior.

The mechanical side, in turn, must receive back:

1. a completed electrical section,
1. wire-routing implications,
1. any required geometry changes caused by clearance or harness constraints.
...
1. wire cost by length,
1. wire weight by gauge and length.
...
1. connectors,
1. wires,
1. motors where they are electrical BOM items rather than only mechanical actuators.
...
1. a schematic view,
1. a 3D wire-route view,
1. the normal model-viewer context,
1. traceable linkage between electrical artifacts and the reviewed episode.
...
1. the power budget is credible,
1. the routed wires are acceptable for the task,
1. the mechanical system behaves correctly under power,
1. no electrical failure occurs during the run,
1. the benchmark objective is satisfied.
...
1. hide routing conflicts behind logical-only wires when the task required physical wiring,
1. pass motion objectives while violating electrical constraints,
1. omit the evidence needed to inspect the electrical design.
...
1. circuit-validation results,
1. pricing and weight that include electrical items,
1. schematic and route visualization when available,
1. simulation results that identify electrical failure modes.
...
1. the schematic,
1. the routed wires when physical wiring matters,
1. the outcome of the circuit-validation gate,
1. any failure classification from the simulation run.
...
1. wire-tear and routing-conflict behavior,
1. full electromechanical workflow,
1. power-budget checks,
1. electrical COTS search and costing,
1. transient analysis when required,
1. fluid-electronics coupling.
...
1. stricter fail-closed behavior in every electrical subpath,
1. richer dynamic circuit analysis when a benchmark truly needs it,
1. more exact cable physics across every backend/runtime mode,
1. stronger explicit handling of legacy implicit-power episodes.
