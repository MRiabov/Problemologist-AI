...
After a number of experiments on evaluation dataset containing minimal problems, some involving one or two prompts, we find that the models are unable to solve even the simplest problems.

We found that failures could be attributed to unclear planning. Planner agents would present a plausible, yet very unspecific plans on how an environment could be solved. As such, during the implementation, the `coder` agents have struggled to define the geometry in such a way that would consistently work; and later during implementation, disregarded the plan as binding and started coming up with own attempts.

As such, we run a conducted a set of experiments of improving the agent intermediate outputs - the exchange (plan handoff) between planner and execution (coder) agents and added plan validation infrastructure in attempt to make the final submission easier to work with. In particular:
\begin{enumerate}
    \item Planners now define a trajectory of the payload - planners have to calculate the exact trajectory of the payload object including its rotations.
    \item Planners must define a valid geometry (but not yet manufacturable)
    \item Planners must define a
\end{enumerate}
