...
This appendix figure combines three rendered images taken from an environment successfully solved by benchmar-planner; the images show RGB (isometric view), depth (isometric view) and segmentation (sideways isometric view) views. All pictures show different views of the same environment. In particular, we can observe that:
- RGB:
  - Guard rails on the side will prevent a ball from falling outside to the forbidden bounds,
  - A platform below the objective that, once the ball will be spawned, will as a ramp, push a ball to the objective.
  - A cover on top to prevent the ball from accidentally moving out of the simulation

- Segmentation and depth:
  - Segmentation and depth maps show some objects translucently allowing to preview the render.
A red line (shown to the agent) showing the planned path of the object from the start of the environment to the end of it.
2. A green objective the object must
\begin{figure}[!htbp]
\centering
