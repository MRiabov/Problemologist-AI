...
\hypertarget{types-of-problems-to-be-solved-are-solving}{%
\paragraph{Types of problems to solve}\label{types-of-problems-to-be-solved-are-solving}}
...
Two important findings allow us to use LLMs for engineering: first, code-CAD has emerged and become readily accessible via tools like build123d {[}4{]}. CodeCAD has existed since the 2000s; however, build123d makes it materially better in many ways and provides more industry-standard frameworks. An important finding over the last three years is that LLMs are significantly superior in using code actions as a way to interact with the environment {[}23{]}, more so than ReAct {[}16{]}. As such, we choose to generate geometric code and other artifacts (e.g.~plans) using agentic coding environments, similar to the OpenHands framework {[}9{]}.
...
\item
  ...
  \item
    Engineering Solution Reviewer - An agent that works in case the Engineering Coder is incapable.
  \end{itemize}
\item
...
\hypertarget{scalabiity-and-adaptibility}{%
\subsubsection{Scalability and adaptability}\label{scalabiity-and-adaptibility}}
...
\hypertarget{adaptibility-and-expensibility}{%
\paragraph{Adaptability and extensibility}\label{adaptibility-and-expensibility}}
