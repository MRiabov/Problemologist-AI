...
This appendix figure combines six rendered images from two versions of the same environment: the engineer-planned render set in the top row and the benchmark-only render set in the bottom row. The source PNGs are archived under \texttt{render\_sources/} for reproducibility. The images show RGB (isometric view), depth (isometric view) and segmentation (sideways isometric view) views. All pictures show different views of the same environments. In particular, we can observe that:
\begin{itemize}
    \item RGB:
    \begin{itemize}
