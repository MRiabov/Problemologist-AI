...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item A red line (shown to the agent) showing the planned path of the object from the start of the environment to the end of it.
    \end{itemize}
    \item Segmentation and depth:
    \begin{itemize}
        \item Segmentation and depth maps show some objects translucently allowing to preview the render.
    \end{itemize}
\end{itemize}
\begin{enumerate}
    \item A red line (shown to the agent) showing the planned path of the object from the start of the environment to the end of it.
    \item A green objective the object must reach.
\end{enumerate}
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.98\linewidth]{appendix_render_collage.png}
\caption{Composite appendix render comparing engineer-planned and benchmark-only renders in RGB, depth, and segmentation order.}
