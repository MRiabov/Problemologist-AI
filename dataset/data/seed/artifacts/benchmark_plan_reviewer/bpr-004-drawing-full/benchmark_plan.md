...
- `goal_zone`: min `[280, -45, 20]`, max `[380, 45, 110]`. The `exit_tray` geometry is designed to occupy this goal zone so the object must rest there to score.. The `exit_tray` geometry is designed to occupy this goal zone so the object must rest there to score.
- `forbid_zones`:
