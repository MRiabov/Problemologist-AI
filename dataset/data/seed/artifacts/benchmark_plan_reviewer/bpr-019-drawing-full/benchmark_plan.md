...
- `goal_brake_tray`: tray centered at `[415, 0, 28]` with size `[150, 120, 32]`; planner notes describe a small tipping brake feature that only engages near the end of travel to bleed speed before the goal tray.
...
- Benchmark-owned motion should read as a single `rotate_x` driven roller benchmark, even though the brake tray concept introduces a small motion near the end of travel.
