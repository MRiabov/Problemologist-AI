...
- `upper_route_wall`: guide wall centered at `[370, 110, 60]` with size `[180, 20, 120]`.
- `lower_route_wall`: guide wall centered at `[370, -110, 60]` with size `[180, 20, 120]`.
...
- `goal_zone`: min `[420, -60, 15]`, max `[520, 60, 110]`; the `goal_catch_tray` geometry is designed to occupy this goal zone so the ball must rest in the tray to score.
- `forbid_zones`:
