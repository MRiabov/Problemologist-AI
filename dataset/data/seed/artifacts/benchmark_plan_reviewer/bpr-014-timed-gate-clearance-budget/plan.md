...
- `gate_pivot_arm`: slender flap centered at `[100, 0, 70]` with size `[18, 120, 90]`; benchmark-side motion is one `rotate_z` axis only, and the open window is trimmed to about `44` mm to make the timing problem sharper.
...
- Benchmark-owned motion must stay limited to one `rotate_z` gate axis with no auxiliary sliders or hidden second motors; the planner assumes the ball will still pass the narrowed gate window during a short open interval.
...
- Fail if the ball exits `simulation_bounds`, clips through undeclared gate geometry, or the benchmark relies on extra motion not declared in `assembly_definition.yaml`.
...
- `assembly_definition.yaml` records the single rotating gate DOF plus benchmark-local costs.
