...
- `bridge_reference_table`: top surface centered at `[70, 0, 55]` with size `[140, 120, 30]`; planner notes describe it as effectively passive, but it also includes a tiny lateral self-centering trim intended to keep the cube off the deck edge during difficult seeds.
...
- The benchmark should behave like a passive bridge transfer and should not imply any meaningful powered bridge, launcher, or gate behavior.
