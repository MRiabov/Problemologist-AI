...
6. Treat `transfer_lane` as the continuous contact path from the roller bed into the goal tray, and keep it unobstructed.
7. The drafting sheet callouts `1`-`6` track the base plate, entry funnel, roller bed, idler guide, goal tray, and the left-side drive motor corridor, respectively.
...
| -- | -- | -- | -- |
| CALC-001 | Capture envelope sizing | `entry_funnel` top opening exceeds the narrow throat by 90 mm, leaving a generous lateral capture margin. | Pass |
| CALC-002 | Lane reach verification | `roller_bed` length of 820 mm keeps the transfer path long enough to hand the ball cleanly into `goal_tray`. | Pass |
| CALC-003 | Base stability check | `base_plate` footprint is long and low, keeping the center of mass near the floor. | Pass |
