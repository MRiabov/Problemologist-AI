...
- `support_tower`: two vertical aluminum towers, one near X=-0.5 m and one near X=+0.5 m.
- `raised_goal_shelf`: two aluminum goal-shelf plates positioned on the support towers near X=+0.5 m.
- `lift_carriage`: two HDPE carriage plates that slide along the Z axis, providing lateral guidance.
