...
- Context: The seed now includes the benchmark objective bounds, the coarse `shelf_lift` motion forecast in `assembly_definition.yaml`, and the narrower `precise_path_definition.yaml` refinement that keeps the same build-safe start and goal-contact finish.
