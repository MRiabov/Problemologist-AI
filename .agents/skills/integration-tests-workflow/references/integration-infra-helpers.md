...
## Shared integration seeding helpers

- `tests/integration/agent/helpers.py`
  - Shared seeding helpers for integration runs, especially `seed_benchmark_assembly_definition(...)` for deterministic benchmark-setup YAML and cost/weight-cap fixtures.
  - Also includes reviewer handoff helpers that seed matching YAML and manifest artifacts for latest-revision review flows.
