...
The dashboard supports two modes: **Real-Time Monitoring** (streaming live agent thoughts and renders) and **Historical Review** (browsing past episodes from the SQLite database). It mimics the layout of modern AI coding assistants (like Cursor) but adds a specialized **3D Viewer Pane** to interactively inspect the CAD models (STLs/Meshes) generated by the agent.
...
*   **Sidebar Navigation**:
    *   **Mode Selection**: Users must be able to switch between "Live Monitoring" and "History Browser" modes.
