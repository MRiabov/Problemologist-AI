...
### User Story 1 - Multi-Level Geometric Selection (Priority: P1)

An engineer can select specific geometric features (faces, edges, vertices), individual parts, or entire subassemblies in the CAD viewer. The system provides a UI toggle to switch between selection modes (Face, Part, Subassembly) and automatically determines the best viewing angle for the selection.

**Why this priority**: Core of steerability; allows precise intervention at different levels of abstraction (from fine geometric tweaks to high-level assembly changes).

**Independent Test**: User can toggle between selection modes in the CAD viewer. Selecting a face, part, or subassembly highlights the target and transmits the correct metadata (IDs, positions) and an optimal isometric screenshot to the agent.

**Acceptance Scenarios**:

1. **Given** a rendered 3D model in the CAD viewer, **When** the user selects "Face Mode", **Then** individual faces can be highlighted and selected.
2. **Given** a rendered 3D model, **When** the user selects "Part Mode", **Then** the entire part containing the clicked feature is highlighted.
3. **Given** a rendered 3D model, **When** the user selects "Subassembly Mode", **Then** the parent subassembly of the clicked part is highlighted.
4. **Given** a selection at any level, **When** the user submits a prompt, **Then** the system automatically selects the best viewing angle (from 8 standard isometric views) and captures a snapshot where the selection is rendered more brightly.
5. **Given** a submitted prompt, **When** the agent receives the turn, **Then** the context contains a YAML-like block with the selection level (Face/Part/Subassembly), unique identifiers, and geometric metadata.
...
- **FR-001**: System MUST integrate `three-cad-viewer` to enable topological selection at multiple levels: faces, edges, parts, and subassemblies.
...
- **FR-003**: System MUST provide a UI toggle to switch between selection modes: Face, Part, and Subassembly.
- **FR-004**: System MUST parse `@filename:line-range` syntax in the chat interface and retrieve the corresponding source code.
- **FR-005**: System MUST provide an autocomplete mechanism for @-mentioning parts from the active assembly BOM.
- **FR-006**: System MUST implement a server-side (or persistent client-side) queue for user prompts sent during agent execution.
- **FR-007**: The Agent MUST be equipped with tools to translate user selections into robust semantic code. Selection metadata MUST be provided in a YAML-like format including level, IDs, centers, normals, and directions.
- **FR-008**: System MUST highlight the referenced code or parts in the UI when the agent acknowledges them in its response.
- **FR-009**: System MUST support per-user steerability memory, allowing the agent to remember and apply user-specific design preferences across multiple turns or sessions.
