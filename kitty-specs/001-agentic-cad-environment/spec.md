...
- **Distributed Execution**:
  ...
  - **Worker Node**: A sandboxed container (Podman) where code is executed, simulation runs, and files are stored.
- **Filesystem**: Agents operate on a `FilesystemMiddleware` backed by **MinIO (S3-compatible)** on the worker node.
...
3. **Persisted State**: All agent interactions are logged to a Postgres database (Controller) and assets are stored in Global S3 (Railway).
...
- **Latency**: Worker execution (linting/simple scripts) overhead < 500ms (excluding network).
- **Resilience**: Long-running simulations (>30s) are orchestrated via **Temporal** (or compatible async callbacks) to handle container preemption.
- **Video Artifacts**: Simulations produce video summaries and structured logs, persisted to S3.
- **Skill Updates**: Skills are strictly read-only for the worker, creating a stable environment derived from the repo or a sidecar learner.
...
├── renders/                    # [Read-Write] Images and Videos generated by code
├── journal.md                  # [Read-Write] Episodic memory
├── todo.md                     # [Read-Write] Planner's TODO list
...
- Abstracts S3 storage as a local filesystem for the agent tools.
...
| `simulate(component) -> SimulationResult` | Commits code to git, runs MuJoCo simulation, returns pass/fail and summary. |
...
- **Databases**:
  ...
  - **S3 (Global)**: Assets (Videos, Images, Git Archives).
  - **S3 (Worker)**: Ephemeral filesystem state.
