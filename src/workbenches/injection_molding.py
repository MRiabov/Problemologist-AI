...
def _check_wall_thickness(
    ...
    locations, index_ray, index_tri = intersector.intersects_location(origins, normals, multiple_hits=False)
    ...
    if len(locations) > 0:
        ...
        if len(too_thin) > 0:
            violations.append(f"Wall thickness too thin: {len(too_thin)} samples < {min_mm}mm")
            logger.warning("wall_too_thin", count=len(too_thin), min_dist=float(distances.min()))
        if len(too_thick) > 0:
            violations.append(f"Wall thickness too thick: {len(too_thick)} samples > {max_mm}mm")
            logger.warning("wall_too_thick", count=len(too_thick), max_dist=float(distances.max()))
...
def _calculate_im_cost(
    ...
    tooling_cost = (
        costs_cfg.get("base_mold_cost", 5000.0)
    ...
    logger.info("im_cost_calculated", tooling=tooling_cost, unit=unit_cost, total=total_cost)
...
def analyze_im(part: Union[Part, Compound], config: ManufacturingConfig) -> WorkbenchResult:
    """
    ...
    logger.info("im_analysis_complete", is_manufacturable=is_manufacturable, violations=len(violations))
