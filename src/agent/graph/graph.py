...
def build_graph():
    ...
    def route_tools(state: AgentState) -> Literal["critic", "actor"]:
        ...
        # So [-1] is ToolMessage.
        # We can check the tool_name in the ToolMessage if available, or look
        # at the preceding AIMessage.

        # A robust way is to look at the last AIMessage's tool calls.
        # But here we are *after* the tools node, so the last message is a
        # ToolMessage (or list of them).
    ...
    # Critic -> conditional (journaler, planner, or end)
    def route_critic(state: AgentState) -> Literal["journaler", "planner", "__end__"]:
        # Check if critic marked success
        if state.get("scratchpad", {}).get("critic_status") == "success":
            return "journaler"
