...
def visualize_event(event):
    ...
    for node_name, updates in event.items():
        ...
        console.print(
            Panel(f"Node Update: [bold]{node_name}[/bold]", border_style=color)
        )
                ...
                if isinstance(msg, AIMessage):
                    ...
                    if msg.tool_calls:
                        for tc in msg.tool_calls:
                            console.print(
                                f"[bold {color}]🛠️  Tool Call: {tc['name']}[/bold {color}]"
                            )
...
def visualize_stream(graph, inputs, config=None):
    ...
    """
    console.print(
        Panel("[bold green]Starting VLM CAD Agent...[/bold green]", title="System")
    )
    ...
    for event in graph.stream(inputs, config=config, stream_mode="updates"):
        for node_name, updates in event.items():
            ...
            console.print(
                Panel(f"Exiting Node: [bold]{node_name}[/bold]", border_style=color)
            )
