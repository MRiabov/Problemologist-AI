...
from bd_warehouse.fastener import HexNut, SocketHeadCapScrew, PlainWasher, BasePartObject as FastenerBase
from bd_warehouse.bearing import SingleRowDeepGrooveBallBearing, Bearing as BearingBase
...
class Indexer:
    ...
    def extract_metadata(self, part_class: Type, size: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Failed to extract metadata for {part_class.__name__} {size}: {e}")
            return None
    ...
    def generate_recipe(self, part_class: Type, params: Dict[str, Any]) -> str:
        ...
        class_name = part_class.__name__
        args_str = ", ".join([f"{k}='{v}'" if isinstance(v, str) else f"{k}={v}" for k, v in params.items()])
