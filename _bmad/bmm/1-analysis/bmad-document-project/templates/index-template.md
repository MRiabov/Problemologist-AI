...
- [Architecture](./architecture-%7B%7Bpart_id%7D%7D.md) - Technical architecture for {{part_name}}
  {{#if has_components}}- [Components](./component-inventory-%7B%7Bpart_id%7D%7D.md) - Component catalog{{/if}}
- [Development Guide](./development-guide-%7B%7Bpart_id%7D%7D.md) - Setup and dev workflow
  {{#if has_api}}- [API Contracts](./api-contracts-%7B%7Bpart_id%7D%7D.md) - API documentation{{/if}}
  {{#if has_data}}- [Data Models](./data-models-%7B%7Bpart_id%7D%7D.md) - Data architecture{{/if}}
  {{/each}}
...
- [{{title}}](%7B%7Bpath%7D%7D) - {{description}}
  {{/each}}
  {{else}}
