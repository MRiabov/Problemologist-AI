...
- **Strict Typing**: All data models in API edges must use strict typing (Pydantic `Strict*` types) to ensure schema validity. In addition, we use Beartype for runtime type checking of performance-critical functions.
...
- **Schema autogeneration**: Controller, Worker and Frontend have their types autogenerated via OpenAPI, dependent on Controller node

### Testing Requirements

- **Backend Logic**: Unit tests with `pytest`.
- **Microservices**: Integration tests using `docker-compose`.
- **Frontend**: Basic coverage and critical user flows (Vitest/Jest). Frontend is currently for debugging and demos, mostly.
