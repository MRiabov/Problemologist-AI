    ...
    try:
        ...
        for _ in range(100):
            ...
            if any(abs(v) > 100.0 for v in data.qvel):
                return SimulationResult(False, "Simulation exploded - check for intersections or poor geometry.")
        ...
        return SimulationResult(False, f"Simulation error: {str(e)}")
        ...
        for i in range(len(solids)):
            for j in range(i + 1, len(solids)):
                ...
                if intersection and intersection.volume > 0.1: # 0.1 mm^3 threshold
                    logger.warning("geometric_intersection_detected", volume=intersection.volume)
                    return False
