...
def _codex_skill_loop_prompt_context(
    *,
    ...
    if simulation_result is not None:
        simulation_lines = [
            f"- Simulation success: {simulation_result.success}",
        ]
...
async def _run_codex_skill_loop(
    *,
    ...
    if codex_trace_artifacts is None or not codex_trace_artifacts.session_id:
        ...
        summary.trigger_reason = "missing Codex session trace"
        return summary, codex_trace_artifacts
    ...
    self_analysis_result = await asyncio.to_thread(
        ...
        output_last_message_path=loop_root / "self_analysis_last_message.md",
    ...
    _record_codex_skill_loop_event(
        workspace_dir=workspace_dir,
        event=SkillSelfReflectionEvent(
            ...
            output_path=(
                ...
                if self_analysis_result.output_last_message_path is not None
                else None
            ),
    ...
    summary.self_analysis_turn = CodexSkillLoopTurn(
        stage="self_analysis",
        session_id=codex_session_id,
        return_code=self_analysis_result.return_code,
        timed_out=self_analysis_result.timed_out,
        prompt_path=self_analysis_prompt_path.as_posix(),
        output_path=(
            self_analysis_result.output_last_message_path.as_posix()
            if self_analysis_result.output_last_message_path is not None
        ...
        failure_reason=(
            "codex skill loop self-analysis timed out"
            if self_analysis_result.timed_out
            else (
                f"codex exited with code {self_analysis_result.return_code}"
                if self_analysis_result.return_code not in {0, None}
                else None
            )
        ),
    )
    ...
    skill_update_result = await asyncio.to_thread(
        ...
        yolo=False,
        timeout_seconds=loop_timeout_seconds,
        output_last_message_path=loop_root / "skill_update_last_message.md",
        reasoning_effort="xhigh",
    )
    if skill_update_result.timed_out or skill_update_result.return_code not in {
        0,
        None,
    }:
        ...
        _record_codex_skill_loop_event(
            ...
            event=SkillUpdateEvent(
                ...
                output_path=(
                    ...
                    if skill_update_result.output_last_message_path is not None
                    else None
                ),
        ...
        summary.skill_update_turn = CodexSkillLoopTurn(
            ...
            failure_reason=(
                "codex skill loop skill update timed out"
                if skill_update_result.timed_out
                else f"codex exited with code {skill_update_result.return_code}"
            ),
        )
        return summary, codex_trace_artifacts
